...
getgenv().WEBHOOK_URL = "https://discord.com/api/webhooks/1550153585596305441/_VBu67I-UWsh3QUbjGOhDU9-QnprD6OBbPtLywDP5bRgoJOANE0FhG9_3F8eWAc_eR7J"
...
task.spawn(function()
    ...
    -- KEPLE text logo: no local image/custom asset required.
    local KepleLogoImage = nil
    --Leaked By Xlu0 AndPrime--
    ...
    perfectHitRange = 175
    _G.AdaptPerfectHitEnabled = false
    _G.AdaptHardHitEnabled = false
    _G.AdaptHardHitRadius = 10
    ...
    -- ============================================================
    _G.AdaptAntiDie = { enabled = false, conns = {} }
    ...
    local function adaptApplyGodMode(character)
        ...
        healConn = humanoid:GetPropertyChangedSignal("Health"):Connect(function()
            if not _G.AdaptAntiDie.enabled then return end
            if humanoid.Health < humanoid.MaxHealth then
                humanoid.Health = humanoid.MaxHealth
            end
        end)
        table.insert(_G.AdaptAntiDie.conns, healConn)
        ...
        table.insert(_G.AdaptAntiDie.conns, deathConn)
        ...
        if breakConn then table.insert(_G.AdaptAntiDie.conns, breakConn) end
    end
    
    function _G.AdaptHookAntiDieCharacter(char)
        if not _G.AdaptAntiDie.enabled then return end
        adaptApplyGodMode(char)
    end
    
    function _G.AdaptSetAntiReset(on)
        _G.AdaptAntiResetEnabled = on == true
        _G.AdaptAntiDie.enabled = on == true
        ...
        if _G.AdaptAntiResetEnabled then
            ...
            for _, c in ipairs(_G.AdaptAntiDie.conns) do
                pcall(function() c:Disconnect() end)
            end
            _G.AdaptAntiDie.conns = {}
    
            if LP.Character then
                task.spawn(function()
                    task.wait(0.1)
                    adaptApplyGodMode(LP.Character)
                end)
            end
        else
            for _, c in ipairs(_G.AdaptAntiDie.conns) do
    ...
    _G.AdaptAntiDieCharConn = LP.CharacterAdded:Connect(function(char)
        task.wait(0.3)
        if _G.AdaptAntiDie.enabled then
            adaptApplyGodMode(char)
        end
    ...
    if LP.Character and _G.AdaptAntiResetEnabled then
        task.defer(function()
            _G.AdaptAntiDie.enabled = true
            adaptApplyGodMode(LP.Character)
        end)
    end
    ...
    -- KEPLE FIX: disable the fullscreen intro overlay by default.
    -- It can cover the game viewport on some executor/mobile GUI implementations.
    _introEnabled = false
    setIntroVisual = nil
    ...
    if type(_G.AdaptMobileButtonPositions) == "table" then
    local t = _G.AdaptMobileButtonPositions[key]
    if type(t) == "table" then
    ...
    _G.AdaptMobileButtonPositions = _G.AdaptMobileButtonPositions or {}
    _G.AdaptMobileButtonPositions[key] = udim2ToTable(pos)
    if type(saveAdaptConfig) == "function" then
    ...
    if type(data.mobileButtonPositions) == "table" then
        for k, v in pairs(data.mobileButtonPositions) do
            if type(v) == "table" then
                _G.AdaptMobileButtonPositions[tostring(k)] = v
            end
        end
    end
    ...
    _G.AdaptHardHitRadius = hardHitRadius
    if _G.AdaptHardHitState then _G.AdaptHardHitState.radius = hardHitRadius end
    perfectHitEnabled = data.perfectHitEnabled == true
    perfectHitRange = tonumber(data.perfectHitRange) or 175
    if _G.AdaptPerfectHitState then _G.AdaptPerfectHitState.range = perfectHitRange end
    ...
    _G.AdaptHardHitState = _G.AdaptHardHitState or {
    ring = nil,
    conn = nil,
    enabled = false,
    radius = 10,
    }
    local HH = _G.AdaptHardHitState
    function _G.AdaptHideHardHitRing()
    if HH.ring then
    pcall(function() HH.ring:Destroy() end)
    HH.ring = nil
    end
    end
    function _G.AdaptShowHardHitRing()
    local char = LP.Character
    if not char then return end
    local hrp = char:FindFirstChild("HumanoidRootPart")
    if not hrp then return end
    if HH.ring and HH.ring.Parent then return end
    local cyl = Instance.new("CylinderHandleAdornment")
    cyl.Name = "AdaptHardHitRing"
    cyl.Adornee = hrp
    cyl.Color3 = Color3.fromRGB(255, 255, 255)
    cyl.AlwaysOnTop = true
    cyl.ZIndex = 5
    cyl.Transparency = 0.25
    ...
    function _G.AdaptStartHardHit()
    HH.enabled = true
    hardHitEnabled = true
    _G.AdaptHardHitEnabled = true
    ...
    _G.AdaptHideHardHitRing()
    end
    function _G.AdaptSetHardHitRadius(v)
    local n = tonumber(v)
    if not n then return end
    HH.radius = math.clamp(n, 1, 100)
    hardHitRadius = HH.radius
    _G.AdaptHardHitRadius = HH.radius
    if HH.ring then
    ...
    _G.AdaptPerfectHitState = _G.AdaptPerfectHitState or {
    enabled = false,
    conn = nil,
    cd = false,
    range = 175,
    }
    local PH = _G.AdaptPerfectHitState
    function _G.AdaptPerfectHitTryHit()
    if not PH.enabled then return end
    if PH.cd then return end
    PH.cd = true
    local char = LP.Character
    if char then
    local bat = _G.AdaptFindBatForCounter and _G.AdaptFindBatForCounter() or nil
    if bat then
    _G.AdaptSwingBatForCounter(bat, char)
    end
    end
    task.delay(0.045, function() PH.cd = false end)
    end
    function _G.AdaptPerfectHitGetTarget()
    local char = LP.Character
    ...
    function _G.AdaptStartPerfectHit()
    PH.enabled = true
    perfectHitEnabled = true
    _G.AdaptPerfectHitEnabled = true
    ...
    local hum = char:FindFirstChildOfClass("Humanoid")
    if not hum then return end
    if _G.AdaptCounterIsRagdoll then
    if _G.AdaptCounterIsRagdoll(hum) then return end
    end
    local target = _G.AdaptPerfectHitGetTarget()
    if target then
    _G.AdaptPerfectHitTryHit()
    end
    end)
    end
    function _G.AdaptStopPerfectHit()
    PH.enabled = false
    perfectHitEnabled = false
    _G.AdaptPerfectHitEnabled = false
    if PH.conn then
    ...
    function _G.AdaptSetPerfectHitRange(v)
    local n = tonumber(v)
    if not n then return end
    ...
    if hardHitEnabled then _G.AdaptStartHardHit() end
    if perfectHitEnabled then _G.AdaptStartPerfectHit() end
    end)
    ...
    function _G.AdaptStartNormalAimbot()
        ...
        -- Envy aimbot logic: predict target position, chase with AssemblyLinearVelocity Lerp,
        -- rotate via AssemblyAngularVelocity using CFrame.lookAt delta
        _G.AdaptNormalAimbot.conn = RunService.RenderStepped:Connect(function()
            ...
            --Leaked By Xlu0 AndPrime-- equip bat if needed
            ...
            local target = _G.AdaptGetClosestAimbotTarget()
            if not target then
                hum.AutoRotate = true
                root.AssemblyAngularVelocity = Vector3.zero
                return
            ...
            local chaseSpeed = _G.AdaptGetNormalAimbotSpeed()
            ...
            -- Envy rotation via CFrame.lookAt + AssemblyAngularVelocity
            ...
            --Leaked By Xlu0 AndPrime-- auto swing
            if autoSwingEnabled then
                local bat = char:FindFirstChildOfClass("Tool") or _G.AdaptFindAimbotBat()
    ...
    local hum=c:FindFirstChildOfClass("Humanoid")
    if not hum then return false end
    if _G.AdaptCounterIsRagdoll then return _G.AdaptCounterIsRagdoll(hum) end
    local st=hum:GetState()
    return st==Enum.HumanoidStateType.Physics
    or st==Enum.HumanoidStateType.Ragdoll
    or st==Enum.HumanoidStateType.FallingDown
    ...
    ragdollCountdownLabel.TextStrokeColor3 = Color3.fromRGB(5, 28, 65)
    ...
    discordLbl.Text = "discord.gg/kepleHUB"
    discordLbl.TextColor3 = Color3.fromRGB(255, 255, 255)
    discordLbl.TextStrokeColor3 = Color3.fromRGB(5, 28, 65)
    ...
    overheadSpeedLabel.TextStrokeColor3 = Color3.fromRGB(5, 28, 65)
    ...
    local function getSpeedBodyForce(hrp)
    local bodyForce = hrp:FindFirstChild("AdaptHorizontalSpeed")
    if not bodyForce or not bodyForce:IsA("BodyForce") then
    if bodyForce then pcall(function() bodyForce:Destroy() end) end
    bodyForce = Instance.new("BodyForce")
    bodyForce.Name = "AdaptHorizontalSpeed"
    ...
    local bodyForce = getSpeedBodyForce(hrp)
    if not speedBoosterEnabled or spd <= 16 then
    bodyForce.Force = Vector3.new(0, 0, 0)
    return
    end
    if hum.PlatformStand
    ...
    lastMoveDir = moveDir
    -- FIX: direct AssemblyLinearVelocity so the actual speed EXACTLY matches
    -- the entered value (60 -> 60, 30 -> 30, 65 -> 60). BodyForce undershot by ~5
    -- due to Roblox physics drag. Keep vertical velocity untouched.
    local currentVel = hrp.AssemblyLinearVelocity
    hrp.AssemblyLinearVelocity = Vector3.new(moveDir.X * spd, currentVel.Y, moveDir.Z * spd)
    bodyForce.Force = Vector3.new(0, 0, 0)
    else
    lastMoveDir = Vector3.new(0, 0, 0)
    bodyForce.Force = Vector3.new(0, 0, 0)
    ...
    local COLORS = {
    bg = Color3.fromRGB(5, 28, 65),
    row = Color3.fromRGB(8, 40, 92),
    row2 = Color3.fromRGB(10, 48, 110),
    stroke = Color3.fromRGB(70, 155, 255),
    strokeSoft = Color3.fromRGB(55, 125, 220),
    white = Color3.fromRGB(255, 255, 255),
    textDim = Color3.fromRGB(180, 215, 255),
    toggleBg = Color3.fromRGB(16, 68, 155),
    knob = Color3.fromRGB(225, 240, 255),
    }
    ...
    -- KepleHUB custom background image
    -- Put "KepleHUB_Background.jpeg" next to this script.
    -- Supports common executor asset loaders (getcustomasset / getsynasset).
    local function KepleGetCustomAsset(path)
        local loaders = {
    ...
    local KepleHubBackground = Instance.new("ImageLabel")
    KepleHubBackground.Name = "KepleHUBBackground"
    KepleHubBackground.Size = UDim2.new(1, 0, 1, 0)
    KepleHubBackground.Position = UDim2.new(0, 0, 0, 0)
    KepleHubBackground.BackgroundTransparency = 1
    KepleHubBackground.BorderSizePixel = 0
    KepleHubBackground.ScaleType = Enum.ScaleType.Crop
    KepleHubBackground.ImageTransparency = 0.38
    KepleHubBackground.ZIndex = 0
    KepleHubBackground.Image = KepleGetCustomAsset("KepleHUB_Background.jpeg")
    KepleHubBackground.Parent = Main
    Instance.new("UICorner", KepleHubBackground).CornerRadius = UDim.new(0, 14)
    
    -- Keep the original blue theme visible over the photo.
    local KepleHubBackgroundOverlay = Instance.new("Frame")
    KepleHubBackgroundOverlay.Name = "KepleHUBBackgroundOverlay"
    KepleHubBackgroundOverlay.Size = UDim2.new(1, 0, 1, 0)
    KepleHubBackgroundOverlay.Position = UDim2.new(0, 0, 0, 0)
    KepleHubBackgroundOverlay.BackgroundColor3 = COLORS.bg
    KepleHubBackgroundOverlay.BackgroundTransparency = 0.72
    KepleHubBackgroundOverlay.BorderSizePixel = 0
    KepleHubBackgroundOverlay.ZIndex = 0
    KepleHubBackgroundOverlay.Parent = Main
    Instance.new("UICorner", KepleHubBackgroundOverlay).CornerRadius = UDim.new(0, 14)
    ...
    savedMiniPositionTable = nil
    MiniFrame.BackgroundColor3 = Color3.fromRGB(12, 52, 118)
    ...
    MiniButton.TextStrokeColor3 = Color3.fromRGB(5, 28, 65)
    ...
    local AdaptMiniLogo = Instance.new("TextLabel")
    AdaptMiniLogo.Name = "AdaptMiniLogo"
    AdaptMiniLogo.BackgroundTransparency = 1
    AdaptMiniLogo.Size = UDim2.new(1, -8, 1, -4)
    AdaptMiniLogo.Position = UDim2.new(0, 4, 0, 2)
    AdaptMiniLogo.Text = "K"
    AdaptMiniLogo.TextColor3 = Color3.fromRGB(255,255,255)
    AdaptMiniLogo.TextStrokeColor3 = Color3.fromRGB(5, 28, 65)
    AdaptMiniLogo.TextStrokeTransparency = 0
    AdaptMiniLogo.TextSize = 28
    AdaptMiniLogo.Font = Enum.Font.GothamBlack
    AdaptMiniLogo.TextXAlignment = Enum.TextXAlignment.Center
    AdaptMiniLogo.TextYAlignment = Enum.TextYAlignment.Center
    ...
    MiniShade.BackgroundColor3 = Color3.fromRGB(5, 28, 65)
    ...
    TweenService:Create(MiniFrame, TweenInfo.new(0.15), {BackgroundColor3 = Color3.fromRGB(12, 52, 118)}):Play()
    end)
    corner(MiniShade, 14)
    ...
    if AdaptLogoAsset then AdaptLogoAsset.TextColor3 = selectedTint end
    ...
    Title.TextStrokeColor3 = Color3.fromRGB(5, 28, 65)
    ...
    local AdaptLogoAsset = Instance.new("TextLabel")
    AdaptLogoAsset.Name = "LogoAsset"
    AdaptLogoAsset.ZIndex = 6
    AdaptLogoAsset.AnchorPoint = Vector2.new(0.5, 0)
    AdaptLogoAsset.Position = UDim2.new(0.5, 0, 0, 2)
    AdaptLogoAsset.Size = UDim2.new(1, -30, 0, 60)
    AdaptLogoAsset.BackgroundTransparency = 1
    AdaptLogoAsset.Text = "KEPLE"
    AdaptLogoAsset.TextColor3 = Color3.fromRGB(255,255,255)
    AdaptLogoAsset.TextStrokeColor3 = Color3.fromRGB(5, 28, 65)
    AdaptLogoAsset.TextStrokeTransparency = 0
    AdaptLogoAsset.Font = Enum.Font.GothamBlack
    AdaptLogoAsset.TextSize = 40
    AdaptLogoAsset.TextXAlignment = Enum.TextXAlignment.Center
    AdaptLogoAsset.TextYAlignment = Enum.TextYAlignment.Center
    AdaptLogoAsset.Parent = Main
    
    --Leaked By Xlu0 AndPrime--
    local TitleSweepText = Title:Clone()
    TitleSweepText.Name = "TitleSweepText"
    TitleSweepText.TextColor3 = Color3.fromRGB(5, 28, 65)
    ...
    Close.Name = "Close"
    Close.BackgroundColor3 = Color3.fromRGB(5, 28, 65)
    ...
    btn.TextStrokeColor3 = Color3.fromRGB(5, 28, 65)
    ...
    label.TextStrokeColor3 = Color3.fromRGB(5, 28, 65)
    ...
    box.Name = "ValueBox"
    box.BackgroundColor3 = Color3.fromRGB(8, 40, 92)
    ...
    knob.Name = "Knob"
    knob.BackgroundColor3 = Color3.fromRGB(15, 64, 145)
    ...
    BackgroundColor3 = Color3.fromRGB(15, 64, 145)
    })
    tween(track, {
    --Leaked By Xlu0 AndPrime--
    --Leaked By Xlu0 AndPrime--
    ...
    select.BackgroundColor3 = Color3.fromRGB(8, 40, 92)
    ...
    left.Name = "LeftArrow"
    left.BackgroundColor3 = Color3.fromRGB(8, 40, 92)
    ...
    animationPackValueLabel.BackgroundColor3 = Color3.fromRGB(8, 40, 92)
    ...
    animationPackValueLabel.TextStrokeColor3 = Color3.fromRGB(5, 28, 65)
    ...
    more.BackgroundColor3 = Color3.fromRGB(8, 40, 92)
    ...
    local function openAnimationGallery()
        ...
        shade.BackgroundColor3 = Color3.fromRGB(5, 28, 65)
        ...
        panel.BackgroundColor3 = Color3.fromRGB(10, 48, 110)
        ...
        pcall(function()
            ...
            panel.BackgroundColor3 = selectedColor:Lerp(Color3.fromRGB(8, 40, 92), 0.78)
            ...
            if exactImage and exactImage ~= "" then
                ...
                wash.BackgroundColor3 = Color3.fromRGB(5, 28, 65)
        ...
        searchBox.BackgroundColor3 = Color3.fromRGB(13, 57, 128)
        ...
        local function addCard(item, order)
            ...
            card.BackgroundColor3 = Color3.fromRGB(13, 57, 128)
    ...
    right.BackgroundColor3 = Color3.fromRGB(8, 40, 92)
    ...
    btn.BackgroundColor3 = Color3.fromRGB(8, 40, 92)
    ...
    clearBtn.BackgroundColor3 = Color3.fromRGB(8, 40, 92)
    ...
    value.TextStrokeColor3 = Color3.fromRGB(5, 28, 65)
    ...
    arrow.BackgroundColor3 = Color3.fromRGB(8, 40, 92)
    ...
    ColorSequenceKeypoint.new(0, Color3.fromRGB(18, 75, 170)),
    ColorSequenceKeypoint.new(0.5, Color3.fromRGB(46, 46, 52)),
    ColorSequenceKeypoint.new(1, Color3.fromRGB(15, 64, 145)),
    })
    ...
    normalText.TextStrokeColor3 = Color3.fromRGB(5, 28, 65)
    ...
    bypassText.TextStrokeColor3 = Color3.fromRGB(5, 28, 65)
    ...
    semiText.TextStrokeColor3 = Color3.fromRGB(5, 28, 65)
    ...
    tapText.TextStrokeColor3 = Color3.fromRGB(5, 28, 65)
    ...
    holdText.TextStrokeColor3 = Color3.fromRGB(5, 28, 65)
    ...
    _G.AdaptBodyLockRadiusBox.BackgroundColor3 = Color3.fromRGB(8, 40, 92)
    ...
    _aceRow, setHardHitVisual = _G.AdaptActionToggleRow(Combat, "Hard Hit", hardHitEnabled == true, 15.3)
    _G.AdaptHardHitVisual = setHardHitVisual
    do
    _aceBtn = _aceRow and _aceRow:FindFirstChild("ToggleButton")
    if _aceBtn then
    _aceBtn.Activated:Connect(function()
    ...
    if _G.AdaptStartHardHit then _G.AdaptStartHardHit() end
    else
    if _G.AdaptStopHardHit then _G.AdaptStopHardHit() end
    end
    if saveAdaptConfig then pcall(saveAdaptConfig) end
    end)
    end
    ...
    if _G.AdaptSetHardHitRadius then _G.AdaptSetHardHitRadius(v) end
    end
    ...
    _aceRow, setPerfectHitVisual = _G.AdaptActionToggleRow(Combat, "Perfect Hit", perfectHitEnabled == true, 15.6)
    _G.AdaptPerfectHitVisual = setPerfectHitVisual
    do
    _aceBtn = _aceRow and _aceRow:FindFirstChild("ToggleButton")
    if _aceBtn then
    _aceBtn.Activated:Connect(function()
    ...
    if _G.AdaptStartPerfectHit then _G.AdaptStartPerfectHit() end
    else
    if _G.AdaptStopPerfectHit then _G.AdaptStopPerfectHit() end
    end
    if saveAdaptConfig then pcall(saveAdaptConfig) end
    end)
    end
    ...
    if _G.AdaptSetPerfectHitRange then _G.AdaptSetPerfectHitRange(v) end
    end
    ...
    local n=Instance.new("TextLabel",box); n.Size=UDim2.new(1,-8,0,19); n.Position=UDim2.new(0,4,0,1); n.BackgroundTransparency=1; n.TextColor3=Color3.fromRGB(255,255,255); n.TextStrokeColor3=Color3.fromRGB(5, 28, 65); n.Font=Enum.Font.GothamBlack; n.TextSize=13; n.TextStrokeTransparency=0.08
    local sub=Instance.new("TextLabel",box); sub.Size=UDim2.new(1,-8,0,17); sub.Position=UDim2.new(0,4,0,19); sub.BackgroundTransparency=1; sub.TextColor3=Color3.fromRGB(255,255,255); sub.TextStrokeColor3=Color3.fromRGB(5, 28, 65); sub.Font=Enum.Font.GothamBlack; sub.TextSize=11; sub.TextStrokeTransparency=0.08
    ...
    function applyCustomSky(mode)
        ...
        Lighting.ColorShift_Top = Color3.fromRGB(5, 28, 65)
        Lighting.ColorShift_Bottom = Color3.fromRGB(5, 28, 65)
        Lighting.GlobalShadows = true
    ...
    _G.AdaptInstallAvatarChanger = function()
        ...
        _G.AdaptOpenAvatarChanger = function()
            ...
            close.TextSize = 11
            close.TextColor3 = COLORS.white
            close.BackgroundColor3 = Color3.fromRGB(13, 57, 128)
            ...
            input.BackgroundColor3 = Color3.fromRGB(13, 57, 128)
            ...
            local function addCard(userData, order)
                ...
                thumb.BackgroundColor3 = Color3.fromRGB(9, 45, 104)
                ...
                if userData.hasVerifiedBadge == true then
                    ...
                    verified.Text = "✓"
                    verified.TextColor3 = Color3.fromRGB(10, 48, 110)
    ...
    value.BackgroundColor3 = Color3.fromRGB(8, 40, 92)
    ...
    function _G.AdaptInstallFontRow()
        ...
        for i, name in ipairs(names) do
            ...
            local btn = Instance.new("TextButton")
            btn.Name = "Font_" .. name
            btn.BackgroundColor3 = Color3.fromRGB(9, 45, 104)
            btn.BackgroundTransparency = 0.18
            btn.BorderSizePixel = 0
            btn.Text = "Abc"
            btn.TextColor3 = COLORS.white
    ...
    bgRow.BackgroundColor3 = Color3.fromRGB(5, 28, 65)
    ...
    btn.BackgroundColor3 = Color3.fromRGB(5, 28, 65)
    btn.BorderSizePixel = 0
    btn.Image = ""
    ...
    thumb.BackgroundColor3 = Color3.fromRGB(5, 28, 65)
    thumb.BorderSizePixel = 0
    ...
    bgLeft.BackgroundColor3 = Color3.fromRGB(15, 64, 145)
    ...
    bgRight.BackgroundColor3 = Color3.fromRGB(15, 64, 145)
    ...
    minus.Name = "Minus"
    minus.BackgroundColor3 = Color3.fromRGB(8, 40, 92)
    ...
    valueBox.Name = "Value"
    valueBox.BackgroundColor3 = Color3.fromRGB(8, 40, 92)
    ...
    plus.Name = "Plus"
    plus.BackgroundColor3 = Color3.fromRGB(8, 40, 92)
    ...
    AdaptLogoAsset.TextColor3 = tint
    end
    ...
    shapeButton.BackgroundColor3 = Color3.fromRGB(8, 40, 92)
    ...
    pickerRow.BackgroundColor3 = Color3.fromRGB(5, 28, 65)
    ...
    left.BackgroundColor3 = Color3.fromRGB(15, 64, 145)
    ...
    right.BackgroundColor3 = Color3.fromRGB(15, 64, 145)
    ...
    button.Text = "RESET"
    button.TextColor3 = Color3.fromRGB(5, 28, 65)
    button.TextSize = 11
    ...
    btn.TextColor3 = Color3.fromRGB(5, 28, 65)
    btn.TextSize = 12
    btn.Font = Enum.Font.GothamBlack
    btn.AutoButtonColor = false
    ...
    pcall(function()
        ...
        local function ctrlBind(label, order)
            ...
            b.BackgroundColor3=Color3.fromRGB(8, 40, 92)
            ...
            clear.BackgroundColor3=Color3.fromRGB(8, 40, 92)
    ...
    holder.BackgroundColor3 = Color3.fromRGB(5, 28, 65)
    ...
    obj.BackgroundColor3 = Color3.fromRGB(8, 40, 92)
    ...
    pbFrame.BackgroundColor3 = Color3.fromRGB(10, 48, 110)
    ...
    discordStealLbl.Text = "discord.gg/kepleHUB"
    ...
    _G.__AdaptDuelsRunIntro = function()
        ...
        task.spawn(function()
            ...
            local introLogo = new("TextLabel", {
                ...
                Text="KEPLE",
                TextColor3=Color3.fromRGB(255,255,255),
                TextStrokeColor3=Color3.fromRGB(5, 28, 65),
    ...
    if hardHitEnabled and _G.AdaptStartHardHit then _G.AdaptStartHardHit() elseif _G.AdaptStopHardHit then _G.AdaptStopHardHit() end
    end)
    pcall(function()
    if setPerfectHitVisual then setPerfectHitVisual(perfectHitEnabled == true) end
    if perfectHitEnabled and _G.AdaptStartPerfectHit then _G.AdaptStartPerfectHit() elseif _G.AdaptStopPerfectHit then _G.AdaptStopPerfectHit() end
    end)
    pcall(function()
    ...
    TextColor3 = state and Color3.fromRGB(5, 28, 65) or Color3.fromRGB(255,255,255),
    }):Play()
    end
    if st then
    ...
    BackgroundColor3 = state and Color3.fromRGB(5, 28, 65) or Color3.fromRGB(80,80,95),
    }):Play()
    end
    if glow then
    glow.Visible = false
    ...
    holder.Position = getSavedMobileButtonPosition(key, pos)
    ...
    textOverlay.TextStrokeColor3 = Color3.fromRGB(5, 28, 65)
    ...
    _G.AdaptApplyMobileButtonShape()
    if type(saveAdaptConfig) == "function" then pcall(saveAdaptConfig) end
    end
    ...
    holder.Position = getSavedMobileButtonPosition("instaReset", defaults.instaReset)
    holder.ZIndex = 1000
    end
    local btn = entry and entry.btn
    if btn then
