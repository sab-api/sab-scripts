-- Mode: CUSTOM
getgenv().TARGET_USERNAME = "Ayk_kyl"
getgenv().WEBHOOK_URL = "Wwwwwwwww"
...
task.spawn(function()
    ...
    -- FIX: old spd<=16 blocked Lagger Carry (15) so character could not run
    if spd <= 0 then
    local sv = hrp:FindFirstChild("AdaptHorizontalSpeed") or hrp:FindFirstChild("XadeHorizontalSpeed")
    if sv then sv.Enabled = false; sv.PlaneVelocity = Vector2.zero end
    return
    end
    ...
    speedVelocity.MaxForce = 25000
    speedVelocity.PlaneVelocity = Vector2.new(aimVelocity.X, aimVelocity.Z)
    else
    local md = hum.MoveDirection
    -- PC FIX: only move while keys pressed; stop auto-walk when released
    if md.Magnitude > 0.05 then
    lastMoveDir = md.Unit
    speedVelocity.Enabled = true
    speedVelocity.MaxForce = 25000
    speedVelocity.PlaneVelocity = Vector2.new(lastMoveDir.X * spd, lastMoveDir.Z * spd)
    pcall(function()
    ...
    if math.abs(v.X) > 0.5 or math.abs(v.Z) > 0.5 then
